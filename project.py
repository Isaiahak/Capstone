...
configuration = Configuration(ejection_time=600, monitor_length=1000, error_length=200)
#-----------------------------------------------#application setup#-----------------------------------------------------------#      
...
if __name__ == "__main__":
    ...
    b1_temp_thread = Monitor_thread(battery_1_temp_queue, temp1_graph_queue, notification_queue, "temperature", 70,30, "temp1",  configuration, timer=0.5,  safety_state=temp1_values[1],value_state=temp1_values[2])
    b1_temp_thread.start()   
    print("b1 temp thread started")
    temp2_values  = read_sensor_info("temp2.csv")
    b2_temp_thread = Monitor_thread(battery_2_temp_queue, temp2_graph_queue, notification_queue,  "temperature", 70,30, "temp2",  configuration, timer=0.5,  safety_state=temp2_values[1],value_state=temp2_values[2])
    b2_temp_thread.start()
    print("b2 temp thread started")
    temp3_values  = read_sensor_info("temp3.csv")
    b3_temp_thread = Monitor_thread(battery_3_temp_queue, temp3_graph_queue, notification_queue, "temperature", 70,30, "temp3", configuration, timer=0.5,  safety_state=temp3_values[1],value_state=temp3_values[2])
    b3_temp_thread.start()
    print("b4 temp thread started")
    temp4_values  = read_sensor_info("temp4.csv")
    b4_temp_thread = Monitor_thread(battery_4_temp_queue, temp4_graph_queue, notification_queue, "temperature", 70,30, "temp4", configuration, timer=0.5,  safety_state=temp4_values[1],value_state=temp4_values[2])
    b4_temp_thread.start()
    print("b4 temp thread started")
    volt1_values  = read_sensor_info("volt1.csv")
    b1_volt_thread = Monitor_thread(battery_1_volt_queue, volt1_graph_queue, notification_queue, "voltage", 4, 0, "volt1", configuration, timer=0.5,  safety_state=volt1_values[1],value_state=volt1_values[2])
    b1_volt_thread.start()
    print("b1 volt thread started")
    volt2_values  = read_sensor_info("volt2.csv")
    b2_volt_thread = Monitor_thread(battery_2_volt_queue, volt2_graph_queue, notification_queue, "voltage", 4, 0, "volt2", configuration, timer=0.5, safety_state=volt2_values[1], value_state=volt2_values[2])
    b2_volt_thread.start()
    print("b2 volt thread started")
    volt3_values  = read_sensor_info("volt3.csv")
    b3_volt_thread = Monitor_thread(battery_3_volt_queue, volt3_graph_queue, notification_queue, "voltage", 4, 0, "volt3", configuration, timer=0.5,  safety_state=volt3_values[1], value_state=volt3_values[2])
    b3_volt_thread.start()
    print("b3 volt thread started")
    volt4_values  = read_sensor_info("volt4.csv")
    b4_volt_thread = Monitor_thread(battery_4_volt_queue, volt4_graph_queue, notification_queue, "voltage", 4, 0, "volt4",configuration, timer=0.5, safety_state=volt4_values[1], value_state=volt4_values[2])
    b4_volt_thread.start()
    print("b4 volt thread started")
    pressure_values  = read_sensor_info("pressure.csv")
    pressure_thread = Monitor_thread(pressure_queue,pressure_graph_queue, notification_queue, "pressure", 100, 50, "pressure", configuration, timer=0.5, safety_state=pressure_values[1],value_state=pressure_values[2])
