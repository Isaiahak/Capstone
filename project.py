...
if __name__ == "__main__":
    ...
    b1_volt_thread = Monitor_thread(battery_1_volt_queue, volt1_graph_queue, notification_queue, "voltage", 4, 0, "volt1", configuration,  safety_state=volt1_values[1],value_state=volt1_values[2])
    b1_volt_thread.start()
    print("b1 volt thread started")
    volt2_values  = read_sensor_info("volt2.csv")
    b2_volt_thread = Monitor_thread(battery_2_volt_queue, volt2_graph_queue, notification_queue, "voltage", 4, 0, "volt2", configuration, safety_state=volt2_values[1], value_state=volt2_values[2])
    b2_volt_thread.start()
    print("b2 volt thread started")
    volt3_values  = read_sensor_info("volt3.csv")
    b3_volt_thread = Monitor_thread(battery_3_volt_queue, volt3_graph_queue, notification_queue, "voltage", 4, 0, "volt3", configuration,  safety_state=volt3_values[1], value_state=volt3_values[2])
    b3_volt_thread.start()
    print("b3 volt thread started")
    volt4_values  = read_sensor_info("volt4.csv")
    b4_volt_thread = Monitor_thread(battery_4_volt_queue, volt4_graph_queue, notification_queue, "voltage", 4, 0, "volt4",configuration, safety_state=volt4_values[1], value_state=volt4_values[2])
    b4_volt_thread.start()
    print("b4 volt thread started")
    pressure_values  = read_sensor_info("pressure.csv")
    pressure_thread = Monitor_thread(pressure_queue,pressure_graph_queue, notification_queue, "pressure", 100, 50, "pressure", configuration, safety_state=pressure_values[1],value_state=pressure_values[2])
